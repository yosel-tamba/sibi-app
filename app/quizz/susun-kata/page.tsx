// app/quizz/susun-kata/page.tsx
"use client";

import { useEffect, useState, useRef } from "react";
import Link from "next/link";
import { ArrowLeft, Play, Info, Timer, Award, CheckCircle2, XCircle, ChevronRight, RotateCcw, Layers } from "lucide-react";
...
export default function SusunKataQuizzPage() {
    ...
    const [soal, setSoal] = useState<SoalSusunKata | null>(null);
    const [loading, setLoading] = useState(true);

    // State manajemen susunan kata oleh user
    ...
    const fetchSoal = (currentRiwayat: RekapKuis[]) => {
        ...
        fetch("http://localhost:5000/quizz/susun-kata?limit=1")
            .then((res) => res.json())
            .then((data) => {
                if (data.length > 0) {
                    const item = data[0];

                    // AMAN: Cek apakah video_url masih berupa string JSON, jika ya lakukan parse
                    let parsedVideoUrl: string[] = [];
                    if (typeof item.video_url === "string") {
                        try {
                            parsedVideoUrl = JSON.parse(item.video_url);
                        } catch (e) {
                            console.error("Gagal melakukan parse video_url string:", e);
                            parsedVideoUrl = [item.video_url]; // Fallback jika format string biasa
                        }
                    ...
                    setSoal({
                        ...item,
                        video_url: parsedVideoUrl // masukkan data yang sudah dipastikan berupa Array
                    });

                    setOpsiKata(item.pilihan_kata || []);
                }
                setLoading(false);
            })
            .catch((err) => {
                console.error("Gagal memuat soal susun kata:", err);
                setLoading(false);
            });
    };
    ...
    // Fungsi interaksi klik kata (memindahkan kata ke susunan kalimat)
    const handlePilihKata = (kata: string) => {
        setKataTerpilih([...kataTerpilih, kata]);
        setOpsiKata(opsiKata.filter((k) => k !== kata));
    };

    // Fungsi membatalkan kata yang sudah disusun
    ...
    const handleNextQuestion = (wasSkipped = false) => {
        ...
        const isCorrect = wasSkipped ? false : susunanUser.trim().toLowerCase() === soal.jawaban_benar.trim().toLowerCase();

        const updateRiwayat = [
            ...riwayat,
            {
                keyword: soal.jawaban_benar,
                ...
                <div className="bg-white/75 backdrop-blur-md border border-slate-200/50 p-8 md:p-10 rounded-3xl shadow-sm text-center">
                    ...
                    <div className="bg-amber-50 border border-amber-200/60 rounded-2xl p-5 text-left mb-8 flex gap-4 items-start">
                        ...
                        <div className="text-sm text-amber-800 leading-relaxed">
                            ...
                            Anda memiliki waktu <b>60 detik</b> untuk memecahkan susunan kalimat dari <b>10 video tantangan</b>. Klik potongan kata untuk menyusun jawaban.
                        </div>
                    </div>
    ...
    if (loading) return <div className="p-8 text-center text-slate-500">Memuat video tantangan...</div>;
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                ...
                {/* KOLOM KIRI: Media Video Player (Opsi A) */}
                ...
                <div className="flex flex-col gap-6">
                            ...
                            {kataTerpilih.length === 0 ? (
                                ...
                                kataTerpilih.map((kata, idx) => (
                                    <button
                                        ...
                                        className="bg-teal-50 border border-teal-200 hover:bg-rose-50 hover:border-rose-200 text-teal-800 hover:text-rose-800 text-sm font-semibold px-4 py-2 rounded-xl transition-all animate-in fade-in zoom-in-95 duration-150 cursor-pointer"
                                    >
                                        {kata}
                                    </button>
                                ))
                    ...
                    <div className="flex gap-4">
                        <button
                            ...
                            className="flex-1 inline-flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white font-bold py-4 rounded-2xl transition-all shadow-lg hover:shadow-teal-500/20 cursor-pointer"
                        >
